...
// (owner-only, matching the desktop sidebar). Inbox is reached from the Home
// header's icon; plan creation lives in the tabs' own buttons. Active item is
// zinc-900, inactive zinc-400 icon / zinc-500 label.
...
export default function DashboardBottomBar({
  ...
  const items = isOwner ? ITEMS : ITEMS.filter((i) => i.id !== "settings");
  return (
