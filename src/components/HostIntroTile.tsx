 ...
 *
 * `HostIntroPlayer` is the open state on its own — the player and the name
 * beside it — for a surface with no cover to lay it over (the /me hero, whose
 * tile is 92px). The caller gives it a box; it fills it.
 */
...
/**
 * The player and the caption beside it, filling whatever box it's given.
 * Autoplays with sound (it only ever mounts from a tap). Clicks inside don't
 * bubble, so a parent can treat any click that reaches it as "close".
 */
export function HostIntroPlayer({
  video,
  hostName,
  hostBio,
  hostAvatar,
  ...
  const landscape = isLandscape(video.aspectRatio);

  return (
    <div className="flex h-full w-full items-end gap-4">
      {/* Portrait takes stand beside the name; a landscape take fills the
          width instead and the side column steps aside — the bio is in the
          sheet anyway, and a 16:9 frame squeezed to the box's height would
          be a crop, which is the thing being fixed. */}
      <div
        className={`relative shrink-0 cursor-pointer overflow-hidden rounded-xl bg-black shadow-[0_16px_40px_rgba(0,0,0,0.5)] ring-2 ring-white ${
          landscape ? "aspect-video max-h-full w-full" : "aspect-[9/16] h-full"
        }`}
        onClick={(e) => {
          e.stopPropagation();
      ...
      <div className={`min-w-0 flex-1 pb-2 text-white ${landscape ? "hidden" : ""}`} onClick={stop}>
        ...
        {closeHint && <p className="mt-3 hidden text-xs text-white/60 md:block">{closeHint}</p>}
      </div>
    </div>
  );
}
